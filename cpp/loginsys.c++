    ...
    if (exists) {
        ...
        while (exists) {
            ...
            if (!exists) {
                ...
                file2 << "username = " << username << " | password = " << password << " | name = " << name << "\n";
                file2.close();
                break;
            }
            number++;
...
void loginUser() {
    bool found = false;
    std::string username, password, name = "", line;
    ...
    std::ifstream file("info.txt");
    while (getline(file, line)) {
        if (line.find("username = " + username + " | password = " + password) != std::string::npos) {
            found = true;
